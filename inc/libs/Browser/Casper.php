<?php
    ...
    class Casper {
        ...
        public function __construct($_path2casper = NULL, $_temp_dir = NULL) {
            if ($_path2casper) $this->_path2casper = $_path2casper;
            if ($_temp_dir) $this->_temp_dir = $_temp_dir;
        }
...
FRAGMENT;
        ...
        public function fillFormSelectors($selector, $data = [], $submit = FALSE) {
            $jsonData = json_encode($data);
            $jsonSubmit = ($submit) ? 'true' : 'false';
            $fragment = <<<FRAGMENT
casper.then(function () {
...
FRAGMENT;
            ...
            exec("export PHANTOMJS_EXECUTABLE=/usr/local/bin/phantomjs; " . $this->_path2casper . 'casperjs ' . $filename . $options, $output);
            //exec($this->_path2casper.'phantomjs '.$this->_path2casper.'casperjs '.$filename.$options, $output);

            if (empty($output)) throw new \Exception('Can not find CasperJS.');
